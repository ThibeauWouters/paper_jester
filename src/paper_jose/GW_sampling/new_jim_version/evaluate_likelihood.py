...
lambda_transform = utils.ChirpMassMassRatioToLambdas(name_mapping)
...
for i in range(N):
    ...
    eos_params = {k: v.at[0].get() for k, v in eos_params.items()}
    ...
    log_L = jax.jit(gw_likelihood.evaluate)(params, None)
    
    np.savez(f"./likelihood_evaluations/{i}.npz", log_L=log_L, params=params, eos_params=eos_params, NS_params=NS_params)
